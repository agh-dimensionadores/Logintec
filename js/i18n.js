/**
 * Logintec – Internacionalización ES / EN
 */
const LogintecI18n = (() => {
  const STORAGE_KEY = 'logintec-lang';

  const translations = {
    es: {
      'meta.title': 'Logintec – Critical Logistics Management',
      'meta.description': 'Logintec es una empresa dedicada al diseño, gestión e implementación de valor agregado en Soluciones Logísticas. Principalmente enfocada en la Logística de Partes de Servicio (SPL) y volumetría.',

      'nav.services': 'Servicios',
      'nav.company': 'Empresa',
      'nav.volumetry': 'Dimensionadores',
      'nav.contact': 'Contáctenos',

      'hero.tagLogistics': 'Logística',
      'hero.tagVolumetry': 'Volumetría',
      'hero.ctaLogistics': 'Servicios de logística',
      'hero.ctaDimensioners': 'Dimensionadores',
      'hero.panel.logistics.eyebrow': '01 · Logística',
      'hero.panel.volumetry.eyebrow': '02 · Volumetría',
      'hero.panel.logistics.chip1': 'SPL',
      'hero.panel.logistics.chip2': 'IOR/EOR',
      'hero.panel.logistics.chip3': 'Puerta a puerta',
      'hero.panel.volumetry.chip1': 'Medición 3D',
      'hero.panel.volumetry.chip2': 'Peso y volumen',
      'hero.panel.volumetry.chip3': 'Integraciones',

      'welcome.title': '<span class="t-bold">Hola y </span><span class="t-accent">bienvenidos!</span>',
      'welcome.p1': 'Logintec es una empresa dedicada al diseño, gestión e implementación de valor agregado en Soluciones Logísticas. Principalmente enfocada en la Logística de Partes de Servicio (SPL), Repuestos críticos y Soluciones de Valor Agregado (VAS), tales como fullfilment, reflashing, screening y distribución.',
      'welcome.p2': 'De todas formas, el fuerte de Logintec se basa en su capacidad de implementar soluciones que se adecuen a las necesidades de los clientes en término de costos y niveles de servicio.',
      'welcome.cta': 'Conocé más de nosotros',
      'welcome.services.label': 'Nuestros servicios',
      'welcome.svc1.title': 'Entregas ASAP',
      'welcome.svc1.desc': 'Servicios de entregas en CABA y AMBA en 2 horas.',
      'welcome.svc2.title': 'Documentación',
      'welcome.svc2.desc': 'Realizamos todos los trámites para que sus productos ingresen al país.',
      'welcome.svc3.title': 'Servicio NFO',
      'welcome.svc3.desc': 'Entregamos en las capitales del interior, embarcando en el primer vuelo disponible.',
      'welcome.svc4.title': 'Retorno de importaciones',
      'welcome.svc4.desc': 'Enviamos sus productos al país de origen para realizar las reparaciones.',
      'welcome.svc5.title': 'Warehousing',
      'welcome.svc5.desc': 'Contamos con un centro de almacenaje para sus productos tecnológicos.',
      'welcome.svc6.title': 'Transporte terrestre',
      'welcome.svc6.desc': 'Contamos con una red que nos permite llegar a toda la República Argentina.',
      'welcome.stat1.title': '+15 AÑOS',
      'welcome.stat1.desc': 'de experiencia en logística crítica',
      'welcome.stat2.title': '24/7',
      'welcome.stat2.desc': 'operación continua sin interrupciones',
      'welcome.stat3.title': '100%',
      'welcome.stat3.desc': 'cobertura nacional en entregas',
      'welcome.stat4.title': 'IOR / EOR',
      'welcome.stat4.desc': 'Importamos y exportamos a nombre de terceros',
      'welcome.card.logistics.title': 'SOLUCIONES LOGÍSTICAS',
      'welcome.card.logistics.desc': 'Logística de servicio de partes (SPL), IOR/EOR, almacenaje y transporte puerta a puerta para repuestos críticos',
      'welcome.card.cubiscan.title': 'DIMENSIONADORES INTELIGENTES',
      'welcome.card.cubiscan.desc': 'Equipos de volumetría para medición precisa, toma de peso y cálculo de volumen en su operación.',
      'welcome.card.more': 'Conocé más',

      'logistics.label': 'Nuestros servicios',
      'logistics.title': '<span class="t-bold">Logística</span> <span class="t-accent">de partes críticas</span>',
      'logistics.desc': 'Diseñamos, gestionamos e implementamos soluciones logísticas de valor agregado para industrias que no pueden detenerse.',
      'logistics.ctaAll': 'Ver todos los servicios',

      'servicios.meta.title': 'Servicios – Logintec',
      'servicios.meta.description': 'Logintec es su mejor solución de Supply Chain por el alcance de sus entregas de partes y componentes de recambio en tiempos críticos: SPL, IOR/EOR y warehousing.',
      'servicios.hero.label': 'Nuestros servicios',
      'servicios.hero.title': 'Logística de partes <span class="t-accent">críticas</span>',
      'servicios.hero.desc': 'Diseñamos, gestionamos e implementamos soluciones logísticas de valor agregado para industrias que no pueden detenerse: SPL, IOR/EOR, almacenaje y transporte puerta a puerta.',
      'servicios.hero.cta': 'Ver soluciones',
      'servicios.pillars.label': 'Soluciones principales',
      'servicios.pillars.title': '<span class="t-bold">Tres pilares</span> <span class="t-accent">para tu operación</span>',
      'servicios.pillars.desc': 'Cobertura integral desde la importación hasta la entrega en sitio, con tiempos críticos y trazabilidad.',
      'servicios.spl.title': 'Service Parts Logistics',
      'servicios.spl.kicker': 'SPL',
      'servicios.spl.lead': 'Sus equipos funcionando, siempre',
      'servicios.spl.p1': 'Logintec es su mejor solución de Supply Chain por el alcance de sus entregas de partes y componentes de recambio en tiempos críticos. Las entregas abarcan servicios en el mismo día para soportar caídas en sistemas, mantenimientos programados, desarrollo de nuevos productos, cambios de hardware durante la noche y logística reversa.',
      'servicios.spl.p2': 'Tanto los servicios terrestres como los aéreos, conjuntamente con el espacio de almacenaje, nos ayudan a proveer soluciones logísticas integrales a nuestros clientes, las cuales diseñamos para que cumplan sus acuerdos de niveles de servicio.',
      'servicios.spl.p3': 'Coordinamos y supervisamos todas las entregas en tiempo crítico hasta el usuario final, a los técnicos e ingenieros, y a los data centers/storages con hardware críticos. Logintec también se integra directamente con otros operadores logísticos fuera del ámbito de las entregas críticas.',
      'servicios.spl.p4': 'La cadena logística completa de Partes de Servicios incluye desde entregas express en 2 o 4 horas (AMBA), programadas, servicios NFO (avión) a las principales capitales del país y NBD (terrestres) a todo el país, recolección de partes defectuosas (logística reversa), entregas para reparación o destrucción, y otros servicios adicionales.',
      'servicios.ior.title': 'Importer & Exporter of Record',
      'servicios.ior.kicker': 'IOR / EOR',
      'servicios.ior.lead': 'Tener una entidad legal en Argentina',
      'servicios.wh.title': 'Warehousing & Transport',
      'servicios.wh.kicker': 'Almacenaje',
      'servicios.wh.lead': 'Cubrimos todo el país',
      'servicios.wh.p1': 'En Logintec conocemos de urgencias. De mañana, de noche, y feriados. Las 24 horas, 7 días a la semana, 365 días al año. Los usuarios finales no pueden esperar a tener sus reemplazos disponibles, por lo tanto contamos con un warehouse en el cual almacenamos la mercadería de nuestros clientes, y así tener un acceso inmediato a las mismas. Tenemos la flexibilidad de almacenar piezas de muy poco tamaño, como unidades de memoria y/o cables, hasta unidades de gran tamaño como racks de servidores.',
      'servicios.wh.p2': 'Tenemos un gran equipo de trabajo cuyo objetivo es cuidar y administrar cada una de las partes críticas que almacenamos, pudiendo así brindar un excelente y rápido servicio, para que al momento que un cliente nos llame y nos solicite entregar un repuesto o recambio, podamos responder de una manera eficaz y eficiente.',
      'servicios.wh.p3': 'Es completamente necesario acompañar el servicio de almacenaje con una red de distribución que permita entregar estos reemplazos en el menor tiempo posible. Para ello ofrecemos servicios de entrega al usuario final que van desde las 2 horas en Capital Federal y el Gran Buenos Aires, hasta las 24/48/72 horas en el interior de la República Argentina.',
      'servicios.ops.label': 'Servicios operativos',
      'servicios.ops.title': '<span class="t-bold">Capacidades</span> <span class="t-accent">día a día</span>',
      'servicios.ops.desc': 'Una red de servicios diseñada para mantener tu operación en marcha, las 24 horas.',
      'servicios.cta.title': '¿Necesitás una solución a medida?',
      'servicios.cta.desc': 'Contanos sobre tu operación y diseñamos el nivel de servicio que necesitás.',

      'service.spl.desc': 'Entregas en 2 a 4 horas en AMBA, servicios NFO y NBD para todo el país. Coordinación con los técnicos para una entrega personalizada.',
      'service.more': 'Ver más',
      'service.spl.moreText': 'Soportamos caídas de sistemas y mantenimientos urgentes con entregas ASAP, NFO y NBD, alineadas a la ventana operativa de cada técnico.',
      'service.ior.desc': 'IOR y EOR para clientes internacionales. Gestionamos toda la documentación para que sus productos ingresen al país sin contratiempos.',
      'service.ior.more': 'Más info',
      'service.less': 'Menos info',
      'service.ior.detail.p1': 'Hemos desarrollado los servicios de IOR & EOR para atender las necesidades de nuestros clientes internacionales.',
      'service.ior.detail.p2': 'En la mayoría de los países es esencial que su empresa tenga presencia fiscal para poder importar productos; y obtener una licencia de importador/exportador requiere mucho tiempo (varios meses) y mucho dinero. Nuestro servicio asiste a las empresas que no cuentan con dicha presencia corporativa, utilizando nuestra entidad de IOR & EOR.',
      'service.ior.detail.p3': 'Logintec ofrece un rápido desarrollo y parametrización para los procesos de importación/exportación de partes y productos críticos; incluyendo:',
      'service.ior.detail.li1': 'Actuar como el importador/exportador para la aduana y en relación a la declaración de los impuestos a la hora de la importación/exportación;',
      'service.ior.detail.li2': 'Ser la entidad responsable frente a las aduanas, a los impuestos y a otras autoridades reguladoras;',
      'service.ior.detail.li3': 'Solicitar las licencias o permisos para las importaciones/exportaciones;',
      'service.ior.detail.li4': 'Mantener los productos la duración mínima requerida bajo la ley local; y',
      'service.ior.detail.li5': 'Completar todos los formularios regulares asociados al pago de tasas e impuestos.',
      'service.wh.desc': 'Contamos con un warehouse de gestión propia para poder optimizar los procesos, y reducir los tiempos de entrega.',
      'service.wh.moreText': 'Ofrecemos almacenaje tecnológico, entregas puerta a puerta en todo el país y CrossDocking para acortar la cadena de suministro de repuestos críticos.',

      'feature.customs': 'Documentación aduanera',
      'feature.nfo': 'Servicio NFO interior',
      'feature.return': 'Retorno de importaciones',
      'feature.network': 'Red terrestre nacional',

      'cubiscan.label': 'Volumetría',
      'cubiscan.title': '<span class="t-bold">Sistemas de</span> <span class="t-accent">dimensionamiento</span> y pesaje',
      'cubiscan.desc': 'Equipos diseñados para aumentar la productividad de su centro de distribución, depósito y operación logística.',
      'cubiscan.hl1': 'Datos precisos de dimensiones y peso',
      'cubiscan.hl2': 'Integración total con WMS, TMS y ERP',
      'cubiscan.hl3': 'Mayor ROI y eficiencia operativa',

      'dim.length': 'Largo',
      'dim.width': 'Ancho',
      'dim.height': 'Alto',
      'dim.weight': 'Peso',

      'product.25.desc': 'Compacto y preciso',
      'product.100.desc': 'Alto rendimiento',
      'product.110.desc': 'Dimensionamiento y pesaje de nueva generación',
      'product.325.desc': 'Industrial',
      'product.150.desc': 'Versátil',
      'product.200ts.desc': 'Transporte',
      'product.200sq.desc': 'Dimensionamiento de bultos y paquetes grandes',
      'product.1200.desc': 'Gran volumen',

      'benefit.precision.title': 'Precisión',
      'benefit.precision.desc': 'Obtención de dimensiones y peso de forma precisa en segundos.',
      'benefit.integration.title': 'Integración',
      'benefit.integration.desc': 'Compatible con WMS, TMS y sistemas ERP.',
      'benefit.roi.title': 'ROI',
      'benefit.roi.desc': 'Reducción de errores de facturación y optimización de espacio.',

      'cubiscan.demo': 'Solicitar demostración',

      'about.label': 'Quiénes somos',
      'about.title': 'Soluciones <span class="t-accent">logísticas</span> a medida',
      'about.p1': '<strong>Logintec</strong> es una empresa dedicada al diseño, gestión e implementación de valor agregado en soluciones logísticas. Principalmente enfocada en la Logística de Partes de Servicio (SPL), repuestos críticos y Soluciones de Valor Agregado (VAS).',
      'about.p2': 'Nuestro fuerte se basa en la capacidad de implementar soluciones que se adecuen a las necesidades de los clientes en términos de costos y niveles de servicio.',
      'about.urgency.title': 'Urgencia',
      'about.urgency.desc': 'De día, de noche y feriados',
      'about.precision.title': 'Precisión',
      'about.precision.desc': 'Niveles de servicio garantizados',
      'about.partnership.title': 'Partnership',
      'about.partnership.desc': 'Trabajamos con las principales marcas del sector',
      'about.card.label': '50% del negocio',
      'about.card.logistics.title': 'Logística',
      'about.card.logistics.desc': 'SPL · IOR/EOR · Warehousing · Transporte nacional',
      'about.card.volumetry.title': 'Volumetría',
      'about.card.volumetry.desc': 'Dimensionamiento · Aforo · Pesaje',
      'about.cta.text': 'Conocé cómo podemos optimizar tu operación logística.',

      'empresa.meta.title': 'Empresa – Logintec',
      'empresa.meta.description': 'Logintec es una empresa dedicada al diseño, gestión e implementación de valor agregado en Soluciones Logísticas. Principalmente enfocada en la Logística de Partes de Servicio (SPL).',
      'empresa.hero.label': 'Sobre nosotros',
      'empresa.hero.title': 'Más de 15 años desafiando a la logística <span class="t-accent">crítica</span>',
      'empresa.hero.p1': 'Logintec es una empresa dedicada al diseño, gestión e implementación de valor agregado en Soluciones Logísticas. Principalmente enfocada en la Logística de Partes de Servicio (SPL), Repuestos críticos y Soluciones de Valor Agregado (VAS), tales como fullfilment, reflashing, screening y distribución.',
      'empresa.hero.p2': 'De todas formas, la capacidad de Logintec para diseñar e implementar soluciones a medida le permite adecuarse a las necesidades de sus clientes en términos de costos y niveles de servicio.',
      'empresa.hero.cta': 'Contactanos',
      'empresa.stat1.title': 'Más de 15 años',
      'empresa.stat1.desc': 'de experiencia',
      'empresa.stat2.title': 'Más de 30 marcas',
      'empresa.stat2.desc': 'atendidas por nosotros',
      'empresa.stat3.title': 'COBERTURA NACIONAL',
      'empresa.stat3.desc': 'operaciones en todo el país',
      'empresa.stat4.title': 'SOPORTE 24/7',
      'empresa.stat4.desc': 'acompañamiento técnico especializado',
      'empresa.values.label': 'Nuestros valores',
      'empresa.value1': 'Compromiso',
      'empresa.value1.desc': 'Con cada entrega crítica',
      'empresa.value2': 'Precisión',
      'empresa.value2.desc': 'En cada proceso logístico',
      'empresa.value3': 'Innovación',
      'empresa.value3.desc': 'Tecnología y mejora continua',
      'empresa.value4': 'Integridad',
      'empresa.value4.desc': 'Transparencia en cada operación',
      'empresa.mission.title': 'Nuestra misión',
      'empresa.mission.text': 'Brindar soluciones logísticas críticas y tecnología de dimensionamiento inteligente que optimicen las operaciones de nuestros clientes, generando valor y relaciones de largo plazo.',
      'empresa.vision.title': 'Nuestra visión',
      'empresa.vision.text': 'Ser líderes en soluciones logísticas y tecnológicas en Latinoamérica, reconocidos por nuestra innovación, calidad de servicio y compromiso con el crecimiento de nuestros clientes.',
      'empresa.presence.title': 'Presencia en todo el país',
      'empresa.presence.desc': 'Operaciones propias y socios estratégicos en los principales puntos logísticos.',
      'empresa.block.ior.title': 'IOR / EOR en Argentina',
      'empresa.block.ior.text': 'Como parte del SPL y del VAS hemos desarrollado en la Argentina la entidad de Importer / Exporter of Record (IOR / EOR). En Logintec creemos que el Top Management deber ser competitivo de forma que nuestras soluciones y diseños sean adecuadas y efectivas.',
      'empresa.block.spl.title': 'SPL y entregas en tiempo crítico',
      'empresa.block.spl.text': 'El SPL y las entregas en tiempo crítico incluyen los servicios de entregas express en 2 o 4 horas en el AMBA (Capital Federal y Gran Buenos Aires), las 24 horas, los 7 días de la semana. Además contamos con distintos niveles de servicios como entregas a horarios programados, servicios NFO (avión) a las principales capitales del país y NBD (terrestres) a todo el país, recolección de partes defectuosas (logística reversa), entregas para reparación o destrucción, y otros servicios adicionales.',
      'empresa.block.experience.title': 'Experiencia y trayectoria',
      'empresa.block.experience.text': 'Contamos con varios años de experiencia en el negocio de la logística, almacenaje, distribución, comercio exterior, y en todo lo concerniente a los servicios de gestión de Supply Chain.',
      'empresa.block.team.title': 'Nuestro equipo',
      'empresa.block.team.text': 'Logintec está gerenciada por un equipo, con ejecutivos y profesionales experimentados en el área. Contamos con amplios conocimientos en Operaciones, IT y Finanzas. Nuestros clientes son grandes compañías del rubro de Seguridad Informática, Networking, Data Storage, Servicios de IT y Comunicaciones entre otros.',

      'contact.label': 'Contáctenos',
      'contact.title': 'Hablemos de su <span class="t-accent">próximo proyecto</span>',
      'contact.desc': 'Logística crítica o equipos de dimensionamiento — estamos listos para ayudarle.',
      'contact.call': 'Llámenos',
      'contact.write': 'Escríbanos',
      'contact.hours': 'Horario',

      'form.name': 'Nombre',
      'form.company': 'Empresa',
      'form.email': 'Email',
      'form.interest': 'Interés',
      'form.message': 'Mensaje',
      'form.namePlaceholder': 'Su nombre',
      'form.companyPlaceholder': 'Su empresa',
      'form.emailPlaceholder': 'email@empresa.com',
      'form.messagePlaceholder': 'Cuéntenos sobre su necesidad...',
      'form.interest.logistics': 'Servicios de Logística',
      'form.interest.cubiscan': 'Equipos Volumetría',
      'form.interest.both': 'Ambos',
      'form.submit': 'Enviar consulta',
      'form.submitting': 'Enviando…',
      'form.success': 'Consulta enviada. Nos pondremos en contacto pronto.',
      'form.activate': 'FormSubmit necesita activación. Revisá info@logintec.com (y spam) y clickeá “Activate Form”. Si no llega ese mail, el dominio lo está bloqueando.',
      'form.needsServer': 'Abrí el sitio con Live Server o en la web publicada (no como archivo .html).',
      'form.mailtoFallback': 'No se pudo enviar automático. Abrimos tu correo con la consulta lista para enviar.',
      'form.error': 'No se pudo enviar. Intentá de nuevo o escribinos a info@logintec.com.',
      'form.captchaError': 'Marcá la casilla “No soy un robot” para continuar.',

      'footer.desc': 'Soluciones logísticas críticas y sistemas de dimensionamiento.',
      'footer.logistics': 'Logística',
      'footer.cubiscan': 'Cubiscan',
      'footer.contact': 'Contacto',
      'footer.equipment': 'Equipos',
      'footer.volumetry': 'Volumetría',
      'footer.demo': 'Demostración',
      'footer.rights': '© 2026 Logintec. Todos los derechos reservados.',

      'dim.meta.title': 'Volumetría – Logintec',
      'dim.meta.description': 'Volumetría Logintec: dimensionadores AGH, Cubiscan, Conlida y CubeTape. Equipos de medición inteligente para optimizar espacio y costos en su operación logística.',
      'dim.intro.label': 'Tecnología de medición inteligente',
      'dim.intro.title': '<span class="dim-hero__title-line">Medición precisa.</span><span class="dim-hero__title-line">Decisiones <span class="t-accent">inteligentes.</span></span>',
      'dim.intro.desc': 'Dimensionadores automáticos de volumen, peso y dimensiones que optimizan el espacio, reducen costos y mejoran la eficiencia en cada etapa de tu operación logística.',
      'dim.intro.stat': 'marcas líderes',
      'dim.intro.cta': 'Solicitar asesoramiento',
      'dim.hero.cta2': 'Ver equipos',
      'dim.hero.f1.title': 'Medición 3D',
      'dim.hero.f1.desc': 'Dimensiones, peso y volumen al instante',
      'dim.hero.f2.title': 'Alta precisión',
      'dim.hero.f2.desc': 'Tecnologías de última generación',
      'dim.hero.f3.title': 'Integración',
      'dim.hero.f3.desc': 'Compatibles con WMS, TMS y ERP',
      'dim.hero.f4.title': 'Confiabilidad',
      'dim.hero.f4.desc': 'Equipos robustos para operación 24/7',
      'dim.hero.stat1.title': 'Marcas líderes',
      'dim.hero.stat1.desc': 'Tecnología de clase mundial',
      'dim.hero.stat2.title': 'equipos',
      'dim.hero.stat2.desc': 'Instalados a nivel mundial',
      'dim.hero.stat3.title': 'Trazabilidad',
      'dim.hero.stat3.desc': 'Datos precisos en tiempo real',
      'dim.hero.stat4.title': 'Operación continua',
      'dim.hero.stat4.desc': 'Equipos diseñados para alto rendimiento',
      'dim.brands.label': 'Distribuidores de marcas',
      'dim.agh.label': 'Dimensionadores por cámaras RGB 3D',
      'dim.agh.headline': 'Medición que impulsa <span class="t-accent">eficiencia y precisión</span>',
      'dim.agh.title': 'AGH',
      'dim.agh.desc': 'Dimensionadores por cámaras RGB 3D, con diseño robusto para entornos industriales exigentes. Mediciones precisas de volumen, peso y dimensiones en segundos.',
      'dim.agh.cta': 'Ver equipos AGH',
      'dim.agh.cta2': 'Ver video',
      'dim.agh.f1.title': 'Alta precisión',
      'dim.agh.f1.desc': 'Medición precisa de volumen, peso y dimensiones',
      'dim.agh.f2.title': 'Rápida medición',
      'dim.agh.f2.desc': 'Resultados en segundos sin detener la operación',
      'dim.agh.f3.title': 'Robustez industrial',
      'dim.agh.f3.desc': 'Equipos diseñados para entornos exigentes y operación continua',
      'dim.agh.f4.title': 'Integración total',
      'dim.agh.f4.desc': 'Compatibles con WMS, TMS y ERP para una logística conectada',
      'dim.agh.float1.title': 'Cámaras RGB 3D',
      'dim.agh.float1.desc': 'Precisión milimétrica',
      'dim.agh.float2.title': 'Fabricación argentina',
      'dim.agh.float2.desc': 'Calidad y confiabilidad garantizada',
      'dim.cubiscan.label': 'Líder mundial en dimensionamiento',
      'dim.cubiscan.headline': 'Medición que optimiza <span class="t-accent">cada envío y pallet</span>',
      'dim.cubiscan.title': 'Cubiscan',
      'dim.cubiscan.desc': 'Soluciones Cubiscan para la medición de paquetes y pallets. Sistemas de dimensionamiento y pesaje de clase mundial para operaciones logísticas de alto rendimiento.',
      'dim.cubiscan.cta': 'Ver equipos Cubiscan',
      'dim.cubiscan.cta2': 'Ver video',
      'dim.cubiscan.f1.title': 'Medición 3D',
      'dim.cubiscan.f1.desc': 'Dimensiones precisas en segundos',
      'dim.cubiscan.f2.title': 'Fácil integración',
      'dim.cubiscan.f2.desc': 'Compatible con WMS y ERP',
      'dim.cubiscan.f3.title': 'Flexibilidad',
      'dim.cubiscan.f3.desc': 'Equipos para todo tipo de carga',
      'dim.cubiscan.f4.title': 'Confiabilidad',
      'dim.cubiscan.f4.desc': 'Equipos robustos para operación 24/7',
      'dim.cubiscan.float1.title': 'Medición 3D precisa',
      'dim.cubiscan.float1.desc': 'Volumen y peso precisos',
      'dim.cubiscan.float2.title': 'Integración total',
      'dim.cubiscan.float2.desc': 'Conectado a tu WMS y ERP',
      'dim.conlida.label': 'Soluciones de medición de alto rendimiento',
      'dim.conlida.headline': 'Eficiencia y precisión <span class="t-accent">para alto volumen</span>',
      'dim.conlida.title': 'Conlida',
      'dim.conlida.desc': 'Soluciones de medición dinámica y estática para operaciones de alto volumen con excelente relación costo-beneficio y soporte local en Argentina.',
      'dim.conlida.cta': 'Ver equipos Conlida',
      'dim.conlida.cta2': 'Ver video',
      'dim.conlida.f1.title': 'Medición dinámica',
      'dim.conlida.f1.desc': 'Captura en movimiento sin frenar la línea',
      'dim.conlida.f2.title': 'Costo-beneficio',
      'dim.conlida.f2.desc': 'Inversión accesible con retorno rápido',
      'dim.conlida.f3.title': 'Soporte local',
      'dim.conlida.f3.desc': 'Asistencia técnica en Argentina',
      'dim.conlida.f4.title': 'Integración total',
      'dim.conlida.f4.desc': 'Compatibles con WMS, TMS y ERP',
      'dim.conlida.float1.title': 'Medición dinámica',
      'dim.conlida.float1.desc': 'Sin detener la operación',
      'dim.conlida.float2.title': 'Soporte en Argentina',
      'dim.conlida.float2.desc': 'Asistencia técnica local',
      'dim.cubetape.label': 'Medición portátil de alta precisión',
      'dim.cubetape.headline': 'Dimensioná en cualquier lugar con <span class="t-accent">agilidad y precisión</span>',
      'dim.cubetape.title': 'CubeTape',
      'dim.cubetape.desc': 'CubeTape es la solución móvil para capturar dimensiones de paquetes y bultos de forma rápida, transmitiendo los datos a tu sistema para optimizar envíos y costos de flete.',
      'dim.cubetape.cta': 'Ver equipos CubeTape',
      'dim.cubetape.cta2': 'Ver video',
      'dim.cubetape.f1.title': 'Medición portátil',
      'dim.cubetape.f1.desc': 'Dimensioná donde estés, sin equipos fijos',
      'dim.cubetape.f2.title': 'Captura rápida',
      'dim.cubetape.f2.desc': 'Largo, ancho y alto en segundos',
      'dim.cubetape.f3.title': 'Conectividad móvil',
      'dim.cubetape.f3.desc': 'Datos vía Bluetooth a tu app o sistema',
      'dim.cubetape.f4.title': 'Ideal para envíos',
      'dim.cubetape.f4.desc': 'Perfecto para operadores logísticos y couriers',
      'dim.cubetape.float1.title': 'Tecnología CubeTape',
      'dim.cubetape.float1.desc': 'Precisión profesional portátil',
      'dim.cubetape.float2.title': 'Fácil de usar',
      'dim.cubetape.float2.desc': 'Diseño ergonómico y ligero',
      'dim.cta.text': '¿No sabés qué equipo necesitás? Nuestro equipo de especialistas te asesora para encontrar la mejor opción para tu operación.',
      'dim.cta.btn': 'Hablá con un especialista',

      'dim.equipos.label': 'Catálogo de equipos',
      'dim.equipos.back': 'Volver a Volumetría',
      'dim.equipos.download': 'Descargar catálogo',
      'dim.equipos.details': 'Ver detalles',
      'dim.equipos.prevImage': 'Imagen anterior',
      'dim.equipos.nextImage': 'Imagen siguiente',
      'dim.equipos.datasheet': 'Ficha técnica',
      'dim.equipos.video': 'Ver video',
      'dim.equipos.requestVideo': 'Solicitar video',
      'dim.equipos.specs': 'Especificaciones',
      'dim.equipos.highlights': 'Destacados',
      'dim.equipos.close': 'Cerrar',
      'dim.equipos.detailNote': '¿Necesitás cotización, demo o integración con tu WMS? Escribinos y te asesoramos.',
      'dim.equipos.brochure': 'Brochure',
      'dim.equipos.viewBrochure': 'Ver folleto',
      'dim.equipos.consult': 'Consultar',
      'dim.equipos.cs25.f1': 'Ideal para e-commerce y estaciones de sobremesa',
      'dim.equipos.cs25.f2': 'Medición rápida de peso y volumen en un solo paso',
      'dim.equipos.cs25.f3': 'Integración sencilla por USB o puerto serial',
      'dim.equipos.cs100.f1': 'Referencia del mercado en centros de distribución',
      'dim.equipos.cs100.f2': 'Alta precisión y repetibilidad en cada medición',
      'dim.equipos.cs100.f3': 'Conectividad USB y Ethernet para tu WMS',
      'dim.equipos.cs110.f1': 'Sucesor moderno del Cubiscan 100 para centros de distribución',
      'dim.equipos.cs110.f2': 'Medición precisa con display integrado',
      'dim.equipos.cs110.f3': 'Compatible con carro móvil y integración WMS',
      'dim.equipos.cs150.f1': 'Ideal para bultos medianos y grandes en depósito',
      'dim.equipos.cs150.f2': 'Plataforma industrial con sensor superior',
      'dim.equipos.cs150.f3': 'Integración lista para WMS y sistemas de gestión',
      'dim.equipos.cs200.f1': 'Pensado para bultos y paquetes de mayor tamaño',
      'dim.equipos.cs200.f2': 'Operación estática confiable en depósitos',
      'dim.equipos.cs200.f3': 'Salida de datos hacia sistemas de gestión',
      'dim.equipos.cs325.f1': 'Dimensiona piezas y formas irregulares',
      'dim.equipos.cs325.f2': 'Tecnología LED infrarrojo para mayor precisión',
      'dim.equipos.cs325.f3': 'Medición en 3–7 segundos',
      'dim.equipos.odc.f1': 'Cámara 3D para objetos y paquetes',
      'dim.equipos.odc.f2': 'Alta precisión sin contacto con el producto',
      'dim.equipos.odc.f3': 'Interfaz VOXEL CAM',
      'dim.equipos.pdc.f1': 'Dimensionamiento de pallets para depósitos',
      'dim.equipos.pdc.f2': 'Ideal para cross-dock y operaciones industriales',
      'dim.equipos.pdc.f3': 'Interfaz VOXEL CAM',
      'dim.equipos.pdl.f1': 'Tecnología LiDAR para pallets',
      'dim.equipos.pdl.f2': 'Medición en menos de 12 segundos',
      'dim.equipos.pdl.f3': 'Interfaz VOXEL CAM',
      'dim.equipos.cld.f1': 'Diseñado para líneas de alto caudal',
      'dim.equipos.cld.f2': 'Medición dinámica sobre cinta transportadora',
      'dim.equipos.cld.f3': 'Excelente relación costo-beneficio',
      'dim.equipos.ct.f1': 'Medición portátil en cualquier punto de la operación',
      'dim.equipos.ct.f2': 'Captura largo, ancho y alto en segundos',
      'dim.equipos.ct.f3': 'Envío de datos por Bluetooth a tu sistema',
      'dim.equipos.help': '¿No sabés cuál elegir? Nuestros especialistas pueden ayudarte a encontrar el dimensionador ideal para tu operación.',
      'dim.equipos.help.cta': 'Hablar con un especialista',
      'dim.equipos.filter.all': 'Todos',
      'dim.equipos.filter.static': 'Estáticos',
      'dim.equipos.filter.dynamic': 'Dinámicos',
      'dim.equipos.filter.desktop': 'Sobremesa',
      'dim.equipos.filter.industrial': 'Industriales',
      'dim.equipos.filter.pallets': 'Pallets',
      'dim.equipos.filter.belt': 'Cintas',
      'dim.equipos.sort.label': 'Ordenar por:',
      'dim.equipos.sort.featured': 'Más vendidos',
      'dim.equipos.sort.name': 'Nombre',
      'dim.equipos.trust1.title': 'Tecnología de precisión',
      'dim.equipos.trust1.desc': 'Mediciones confiables y repetibles.',
      'dim.equipos.trust2.title': 'Integración total',
      'dim.equipos.trust2.desc': 'Compatible con WMS, TMS y sistemas de automatización.',
      'dim.equipos.trust3.title': 'Soporte especializado',
      'dim.equipos.trust3.desc': 'Acompañamiento técnico antes y después de la compra.',
      'dim.equipos.trust4.title': 'Respaldo Logintec',
      'dim.equipos.trust4.desc': 'Más de 15 años brindando soluciones logísticas.',
      'dim.equipos.agh.pageTitle': 'Equipos AGH – Logintec',
      'dim.equipos.agh.meta': 'Equipos AGH de dimensionamiento por cámaras RGB 3D. Catálogo Logintec.',
      'dim.equipos.agh.title': 'Equipos AGH',
      'dim.equipos.agh.desc': 'Dimensionadores por cámaras RGB 3D de alta precisión para entornos industriales exigentes.',
      'dim.equipos.agh.count': '3 equipos disponibles',
      'dim.equipos.agh.odc55': 'Dimensionador por cámaras RGB 3D para medición rápida de volumen, peso y dimensiones en operación industrial.',
      'dim.equipos.agh.odc': '<span class="eq-card__text-title">Object Dimensioner Camera</span><span class="eq-card__text-desc">medición precisa de objetos y paquetes.</span>',
      'dim.equipos.agh.pdc': '<span class="eq-card__text-title">Pallet Dimensioner Camera</span><span class="eq-card__text-desc">volumetría real de pallets para depósitos y operaciones industriales.</span>',
      'dim.equipos.agh.pdl': '<span class="eq-card__text-title">Pallet Dimensioner LiDAR</span><span class="eq-card__text-desc">dimensionamiento de pallets con tecnología LiDAR para geometrías complejas.</span>',
      'dim.equipos.agh.p1': 'Ideal para medición precisa de paquetes medianos.',
      'dim.equipos.agh.p2': 'Diseñado para operaciones de alto volumen y entornos exigentes.',
      'dim.equipos.agh.p3': 'Medición en movimiento sin detener la línea de producción.',
      'dim.equipos.agh.p4': 'Solución compacta para espacios reducidos y e-commerce.',
      'dim.equipos.agh.p5': 'Dimensionamiento y pesaje de pallets para depósitos y cross-dock.',
      'dim.equipos.agh.p6': 'Integración en cinta transportadora para flujo continuo.',
      'dim.equipos.cubiscan.pageTitle': 'Equipos Cubiscan – Logintec',
      'dim.equipos.cubiscan.meta': 'Equipos Cubiscan de dimensionamiento y pesaje. Catálogo Logintec.',
      'dim.equipos.cubiscan.title': 'Equipos Cubiscan',
      'dim.equipos.cubiscan.desc': 'Sistemas de dimensionamiento y pesaje para paquetes, bultos y pallets de alto rendimiento.',
      'dim.equipos.cubiscan.count': '6 equipos disponibles',
      'dim.equipos.conlida.pageTitle': 'Equipos Conlida – Logintec',
      'dim.equipos.conlida.meta': 'Equipos Conlida de dimensionamiento. Catálogo Logintec.',
      'dim.equipos.conlida.title': 'Equipos Conlida',
      'dim.equipos.conlida.desc': 'Soluciones de medición dinámica y estática para operaciones de alto volumen.',
      'dim.equipos.conlida.count': '1 equipo disponible',
      'dim.equipos.conlida.cld8000': 'Sistema de dimensionamiento para alto caudal, con excelente relación costo-beneficio.',
      'dim.equipos.cubetape.pageTitle': 'Equipos CubeTape – Logintec',
      'dim.equipos.cubetape.meta': 'Equipos CubeTape de medición portátil. Catálogo Logintec.',
      'dim.equipos.cubetape.title': 'Equipos CubeTape',
      'dim.equipos.cubetape.desc': 'Medición portátil de dimensiones para operadores logísticos y couriers.',
      'dim.equipos.cubetape.count': '1 equipo disponible',
      'dim.equipos.cubetape.model': 'Cinta inteligente para capturar largo, ancho y alto y enviar los datos a tu sistema.',

      'aria.openMenu': 'Abrir menú',
      'aria.langSwitch': 'Seleccionar idioma',
    },

    en: {
      'meta.title': 'Logintec – Critical Logistics Management',
      'meta.description': 'Logintec designs, manages and implements value-added logistics solutions. Focused on Service Parts Logistics (SPL) and volumetry.',

      'nav.services': 'Services',
      'nav.company': 'Company',
      'nav.volumetry': 'Dimensioners',
      'nav.contact': 'Contact Us',

      'hero.tagLogistics': 'Logistics',
      'hero.tagVolumetry': 'Volumetry',
      'hero.ctaLogistics': 'Logistics services',
      'hero.ctaDimensioners': 'Dimensioners',
      'hero.panel.logistics.eyebrow': '01 · Logistics',
      'hero.panel.volumetry.eyebrow': '02 · Volumetry',
      'hero.panel.logistics.chip1': 'SPL',
      'hero.panel.logistics.chip2': 'IOR/EOR',
      'hero.panel.logistics.chip3': 'Door to door',
      'hero.panel.volumetry.chip1': '3D measurement',
      'hero.panel.volumetry.chip2': 'Weight & volume',
      'hero.panel.volumetry.chip3': 'Integrations',

      'welcome.title': '<span class="t-bold">Hello and </span><span class="t-accent">welcome!</span>',
      'welcome.p1': 'Logintec is a company dedicated to designing, managing and implementing value-added Logistics Solutions. Mainly focused on Service Parts Logistics (SPL), critical spare parts and Value Added Services (VAS), such as fulfillment, reflashing, screening and distribution.',
      'welcome.p2': 'Our strength lies in implementing solutions tailored to client needs in terms of cost and service levels.',
      'welcome.cta': 'Learn more about us',
      'welcome.services.label': 'Our services',
      'welcome.svc1.title': 'ASAP Deliveries',
      'welcome.svc1.desc': 'Delivery services in CABA and Greater Buenos Aires within 2 hours.',
      'welcome.svc2.title': 'Documentation',
      'welcome.svc2.desc': 'We handle all procedures so your products can enter the country.',
      'welcome.svc3.title': 'NFO Service',
      'welcome.svc3.desc': 'We deliver to provincial capitals, boarding on the first available flight.',
      'welcome.svc4.title': 'Import returns',
      'welcome.svc4.desc': 'We ship your products back to the country of origin for repairs.',
      'welcome.svc5.title': 'Warehousing',
      'welcome.svc5.desc': 'We have a storage center for your technology products.',
      'welcome.svc6.title': 'Ground transport',
      'welcome.svc6.desc': 'Our network allows us to reach every corner of Argentina.',
      'welcome.stat1.title': '+15 YEARS',
      'welcome.stat1.desc': 'of experience in critical logistics',
      'welcome.stat2.title': '24/7',
      'welcome.stat2.desc': 'continuous operation without interruptions',
      'welcome.stat3.title': '100%',
      'welcome.stat3.desc': 'national delivery coverage',
      'welcome.stat4.title': 'IOR / EOR',
      'welcome.stat4.desc': 'We import and export on behalf of third parties',
      'welcome.card.logistics.title': 'LOGISTICS SOLUTIONS',
      'welcome.card.logistics.desc': 'Service Parts Logistics (SPL), IOR/EOR, warehousing and door-to-door transport for critical spare parts',
      'welcome.card.cubiscan.title': 'SMART DIMENSIONERS',
      'welcome.card.cubiscan.desc': 'Volumetry equipment for precise measurement, weighing and volume calculation in your operation.',
      'welcome.card.more': 'Learn more',

      'logistics.label': 'Our services',
      'logistics.title': '<span class="t-bold">Critical parts</span> <span class="t-accent">logistics</span>',
      'logistics.desc': 'We design, manage and implement value-added logistics solutions for industries that cannot afford downtime.',
      'logistics.ctaAll': 'View all services',

      'servicios.meta.title': 'Services – Logintec',
      'servicios.meta.description': 'Logintec is your best Supply Chain solution for critical spare parts and components delivery: SPL, IOR/EOR and warehousing.',
      'servicios.hero.label': 'Our services',
      'servicios.hero.title': 'Critical parts <span class="t-accent">logistics</span>',
      'servicios.hero.desc': 'We design, manage and implement value-added logistics solutions for industries that cannot stop: SPL, IOR/EOR, warehousing and door-to-door transport.',
      'servicios.hero.cta': 'Explore solutions',
      'servicios.pillars.label': 'Core solutions',
      'servicios.pillars.title': '<span class="t-bold">Three pillars</span> <span class="t-accent">for your operation</span>',
      'servicios.pillars.desc': 'End-to-end coverage from import to on-site delivery, with critical SLAs and full traceability.',
      'servicios.spl.title': 'Service Parts Logistics',
      'servicios.spl.kicker': 'SPL',
      'servicios.spl.lead': 'Your equipment running, always',
      'servicios.spl.p1': 'Logintec is your best Supply Chain solution thanks to the reach of its critical-time spare parts and replacement component deliveries. Same-day services support system outages, scheduled maintenance, new product rollouts, overnight hardware changes and reverse logistics.',
      'servicios.spl.p2': 'Both ground and air services, together with warehousing capacity, help us deliver end-to-end logistics solutions designed to meet our clients service-level agreements.',
      'servicios.spl.p3': 'We coordinate and oversee every critical-time delivery to the end user, technicians and engineers, and to data centers/storage sites with critical hardware. Logintec also integrates directly with other logistics operators beyond critical deliveries.',
      'servicios.spl.p4': 'The full Service Parts logistics chain includes express deliveries in 2 or 4 hours (AMBA), scheduled services, NFO (air) to major cities nationwide and NBD (ground) across the country, collection of defective parts (reverse logistics), deliveries for repair or destruction, and other value-added services.',
      'servicios.ior.title': 'Importer & Exporter of Record',
      'servicios.ior.kicker': 'IOR / EOR',
      'servicios.ior.lead': 'A legal entity in Argentina',
      'servicios.wh.title': 'Warehousing & Transport',
      'servicios.wh.kicker': 'Warehousing',
      'servicios.wh.lead': 'We cover the entire country',
      'servicios.wh.p1': 'At Logintec we understand urgency. Morning, night and holidays. 24 hours a day, 7 days a week, 365 days a year. End users cannot wait for replacements, so we operate a warehouse where we store our clients goods for immediate access. We flexibly store everything from very small parts such as memory units and cables to large units such as server racks.',
      'servicios.wh.p2': 'We have a strong team dedicated to caring for and managing every critical part we store, so we can deliver an excellent, fast service: when a client calls for a spare or replacement, we respond effectively and efficiently.',
      'servicios.wh.p3': 'Warehousing must go hand in hand with a distribution network that delivers replacements as fast as possible. That is why we offer end-user delivery from 2 hours in Buenos Aires City and Greater Buenos Aires to 24/48/72 hours across inland Argentina.',
      'servicios.ops.label': 'Operational services',
      'servicios.ops.title': '<span class="t-bold">Day-to-day</span> <span class="t-accent">capabilities</span>',
      'servicios.ops.desc': 'A service network designed to keep your operation running 24/7.',
      'servicios.cta.title': 'Need a tailored solution?',
      'servicios.cta.desc': 'Tell us about your operation and we will design the service level you need.',

      'service.spl.desc': 'Deliveries in 2 to 4 hours in AMBA, NFO and NBD services nationwide. Coordination with technicians for personalized delivery.',
      'service.more': 'See more',
      'service.spl.moreText': 'We support system outages and urgent maintenance with ASAP, NFO and NBD deliveries aligned to each technician’s operating window.',
      'service.ior.desc': 'IOR and EOR for international clients. We handle all documentation so your products enter the country without setbacks.',
      'service.ior.more': 'More info',
      'service.less': 'Less info',
      'service.ior.detail.p1': 'We developed IOR & EOR services to meet the needs of our international clients.',
      'service.ior.detail.p2': 'In most countries it is essential for your company to have a fiscal presence in order to import products; and obtaining an importer/exporter license takes a long time (several months) and a lot of money. Our service assists companies that do not have such a corporate presence by using our IOR & EOR entity.',
      'service.ior.detail.p3': 'Logintec offers rapid setup and parameterization for import/export processes of critical parts and products, including:',
      'service.ior.detail.li1': 'Acting as the importer/exporter for customs and for tax declarations at the time of import/export;',
      'service.ior.detail.li2': 'Being the responsible entity before customs, tax authorities and other regulators;',
      'service.ior.detail.li3': 'Requesting licenses or permits for imports/exports;',
      'service.ior.detail.li4': 'Keeping products for the minimum period required under local law; and',
      'service.ior.detail.li5': 'Completing all standard forms associated with the payment of fees and taxes.',
      'service.wh.desc': 'We operate our own warehouse to optimize processes and reduce delivery times.',
      'service.wh.moreText': 'We offer tech warehousing, door-to-door deliveries nationwide and cross-docking to shorten the critical spare-parts supply chain.',

      'feature.customs': 'Customs documentation',
      'feature.nfo': 'Domestic NFO service',
      'feature.return': 'Import returns',
      'feature.network': 'National ground network',

      'cubiscan.label': 'Volumetry',
      'cubiscan.title': '<span class="t-bold">Dimensioning</span> <span class="t-accent">and weighing</span> systems',
      'cubiscan.desc': 'Equipment designed to increase productivity in your distribution center, warehouse and logistics operation.',
      'cubiscan.hl1': 'Precise dimension and weight data',
      'cubiscan.hl2': 'Full integration with WMS, TMS and ERP',
      'cubiscan.hl3': 'Higher ROI and operational efficiency',

      'dim.length': 'Length',
      'dim.width': 'Width',
      'dim.height': 'Height',
      'dim.weight': 'Weight',

      'product.25.desc': 'Compact and precise',
      'product.100.desc': 'High performance',
      'product.110.desc': 'Next-generation dimensioning and weighing',
      'product.325.desc': 'Industrial',
      'product.150.desc': 'Versatile',
      'product.200ts.desc': 'Transport',
      'product.200sq.desc': 'Dimensioning for large parcels and packages',
      'product.1200.desc': 'High volume',

      'benefit.precision.title': 'Precision',
      'benefit.precision.desc': 'Obtaining dimensions and weight accurately in seconds.',
      'benefit.integration.title': 'Integration',
      'benefit.integration.desc': 'Compatible with WMS, TMS and ERP systems.',
      'benefit.roi.title': 'ROI',
      'benefit.roi.desc': 'Reduced billing errors and space optimization.',

      'cubiscan.demo': 'Request a demo',

      'about.label': 'Who we are',
      'about.title': 'Tailored <span class="t-accent">logistics</span> solutions',
      'about.p1': '<strong>Logintec</strong> is a company dedicated to designing, managing and implementing value-added logistics solutions. Mainly focused on Service Parts Logistics (SPL), critical spare parts and Value Added Services (VAS).',
      'about.p2': 'Our strength lies in implementing solutions tailored to client needs in terms of cost and service levels.',
      'about.urgency.title': 'Urgency',
      'about.urgency.desc': 'Day, night and holidays',
      'about.precision.title': 'Precision',
      'about.precision.desc': 'Guaranteed service levels',
      'about.partnership.title': 'Partnership',
      'about.partnership.desc': 'We work with the leading brands in the industry',
      'about.card.label': '50% of our business',
      'about.card.logistics.title': 'Logistics',
      'about.card.logistics.desc': 'SPL · IOR/EOR · Warehousing · National transport',
      'about.card.volumetry.title': 'Volumetry',
      'about.card.volumetry.desc': 'Dimensioning · Gauging · Weighing',
      'about.cta.text': 'See how we can optimize your logistics operation.',

      'empresa.meta.title': 'Company – Logintec',
      'empresa.meta.description': 'Logintec designs, manages and implements value-added logistics solutions. Focused on Service Parts Logistics (SPL).',
      'empresa.hero.label': 'About us',
      'empresa.hero.title': 'Over 15 years challenging <span class="t-accent">critical</span> logistics',
      'empresa.hero.p1': 'Logintec is a company dedicated to designing, managing and implementing value-added Logistics Solutions. Mainly focused on Service Parts Logistics (SPL), critical spare parts and Value Added Services (VAS), such as fulfillment, reflashing, screening and distribution.',
      'empresa.hero.p2': 'In any case, Logintec\'s ability to design and implement tailored solutions allows it to adapt to its clients\' needs in terms of cost and service levels.',
      'empresa.hero.cta': 'Contact us',
      'empresa.stat1.title': 'Over 15 years',
      'empresa.stat1.desc': 'of experience',
      'empresa.stat2.title': 'Over 30 brands',
      'empresa.stat2.desc': 'served by us',
      'empresa.stat3.title': 'NATIONAL COVERAGE',
      'empresa.stat3.desc': 'operations nationwide',
      'empresa.stat4.title': '24/7 SUPPORT',
      'empresa.stat4.desc': 'specialized technical assistance',
      'empresa.values.label': 'Our values',
      'empresa.value1': 'Commitment',
      'empresa.value1.desc': 'With every critical delivery',
      'empresa.value2': 'Precision',
      'empresa.value2.desc': 'In every logistics process',
      'empresa.value3': 'Innovation',
      'empresa.value3.desc': 'Technology and continuous improvement',
      'empresa.value4': 'Integrity',
      'empresa.value4.desc': 'Transparency in every operation',
      'empresa.mission.title': 'Our mission',
      'empresa.mission.text': 'To provide critical logistics solutions and smart dimensioning technology that optimize our clients\' operations, generating value and long-term relationships.',
      'empresa.vision.title': 'Our vision',
      'empresa.vision.text': 'To be leaders in logistics and technology solutions in Latin America, recognized for our innovation, service quality and commitment to our clients\' growth.',
      'empresa.presence.title': 'Nationwide presence',
      'empresa.presence.desc': 'Own operations and strategic partners at major logistics hubs.',
      'empresa.block.ior.title': 'IOR / EOR in Argentina',
      'empresa.block.ior.text': 'As part of SPL and VAS we developed the Importer / Exporter of Record (IOR / EOR) entity in Argentina. At Logintec we believe top management must be competitive so our solutions and designs are appropriate and effective.',
      'empresa.block.spl.title': 'SPL and time-critical deliveries',
      'empresa.block.spl.text': 'SPL and time-critical deliveries include express delivery services in 2 or 4 hours in Greater Buenos Aires, 24 hours a day, 7 days a week. We also offer scheduled deliveries, NFO (air) services to main provincial capitals and NBD (ground) nationwide, defective parts collection (reverse logistics), repair or destruction deliveries, and other additional services.',
      'empresa.block.experience.title': 'Experience and track record',
      'empresa.block.experience.text': 'We have many years of experience in logistics, warehousing, distribution, foreign trade, and all aspects of Supply Chain management services.',
      'empresa.block.team.title': 'Our team',
      'empresa.block.team.text': 'Logintec is managed by a team of experienced executives and professionals. We have extensive knowledge in Operations, IT and Finance. Our clients include major companies in Cybersecurity, Networking, Data Storage, IT Services and Communications, among others.',

      'contact.label': 'Contact Us',
      'contact.title': "Let's talk about your <span class=\"t-accent\">next project</span>",
      'contact.desc': 'Critical logistics or dimensioning equipment — we are ready to help.',
      'contact.call': 'Call us',
      'contact.write': 'Email us',
      'contact.hours': 'Hours',

      'form.name': 'Name',
      'form.company': 'Company',
      'form.email': 'Email',
      'form.interest': 'Interest',
      'form.message': 'Message',
      'form.namePlaceholder': 'Your name',
      'form.companyPlaceholder': 'Your company',
      'form.emailPlaceholder': 'email@company.com',
      'form.messagePlaceholder': 'Tell us about your needs...',
      'form.interest.logistics': 'Logistics Services',
      'form.interest.cubiscan': 'Volumetry Equipment',
      'form.interest.both': 'Both',
      'form.submit': 'Send inquiry',
      'form.submitting': 'Sending…',
      'form.success': 'Inquiry sent. We will get back to you soon.',
      'form.activate': 'FormSubmit needs activation. Check info@logintec.com (and spam) and click “Activate Form”. If that email never arrives, the domain is blocking it.',
      'form.needsServer': 'Open the site with Live Server or the published URL (not as a local .html file).',
      'form.mailtoFallback': 'Automatic send failed. We opened your email app with the inquiry ready to send.',
      'form.error': 'Could not send. Please try again or email info@logintec.com.',
      'form.captchaError': 'Please check the “I’m not a robot” box to continue.',

      'footer.desc': 'Critical logistics solutions and dimensioning systems.',
      'footer.logistics': 'Logistics',
      'footer.cubiscan': 'Cubiscan',
      'footer.contact': 'Contact',
      'footer.equipment': 'Equipment',
      'footer.volumetry': 'Volumetry',
      'footer.demo': 'Demo',
      'footer.rights': '© 2026 Logintec. All rights reserved.',

      'dim.meta.title': 'Volumetry – Logintec',
      'dim.meta.description': 'Logintec volumetry: AGH, Cubiscan, Conlida and CubeTape dimensioners. Smart measurement equipment to optimize space and costs in your logistics operation.',
      'dim.intro.label': 'Smart measurement technology',
      'dim.intro.title': '<span class="dim-hero__title-line">Precise measurement.</span><span class="dim-hero__title-line">Smart <span class="t-accent">decisions.</span></span>',
      'dim.intro.desc': 'Automatic volume, weight and dimension scanners that optimize space, reduce costs and improve efficiency at every stage of your logistics operation.',
      'dim.intro.stat': 'leading brands',
      'dim.intro.cta': 'Request consultation',
      'dim.hero.cta2': 'View equipment',
      'dim.hero.f1.title': '3D measurement',
      'dim.hero.f1.desc': 'Dimensions, weight and volume instantly',
      'dim.hero.f2.title': 'High precision',
      'dim.hero.f2.desc': 'Latest-generation technologies',
      'dim.hero.f3.title': 'Integration',
      'dim.hero.f3.desc': 'Compatible with WMS, TMS and ERP',
      'dim.hero.f4.title': 'Reliability',
      'dim.hero.f4.desc': 'Robust equipment for 24/7 operation',
      'dim.hero.stat1.title': 'Leading brands',
      'dim.hero.stat1.desc': 'World-class technology',
      'dim.hero.stat2.title': 'units',
      'dim.hero.stat2.desc': 'Installed worldwide',
      'dim.hero.stat3.title': 'Traceability',
      'dim.hero.stat3.desc': 'Accurate real-time data',
      'dim.hero.stat4.title': 'Continuous operation',
      'dim.hero.stat4.desc': 'Equipment designed for high performance',
      'dim.brands.label': 'Brand distributors',
      'dim.agh.label': 'RGB 3D camera dimensioners',
      'dim.agh.headline': 'Measurement that drives <span class="t-accent">efficiency and precision</span>',
      'dim.agh.title': 'AGH',
      'dim.agh.desc': 'RGB 3D camera dimensioners with robust design for demanding industrial environments. Precise volume, weight and dimension measurements in seconds.',
      'dim.agh.cta': 'View AGH equipment',
      'dim.agh.cta2': 'Watch video',
      'dim.agh.f1.title': 'High precision',
      'dim.agh.f1.desc': 'Precise volume, weight and dimension measurement',
      'dim.agh.f2.title': 'Fast measurement',
      'dim.agh.f2.desc': 'Results in seconds without stopping operations',
      'dim.agh.f3.title': 'Industrial robustness',
      'dim.agh.f3.desc': 'Equipment designed for demanding environments and continuous operation',
      'dim.agh.f4.title': 'Full integration',
      'dim.agh.f4.desc': 'Compatible with WMS, TMS and ERP for connected logistics',
      'dim.agh.float1.title': 'RGB 3D cameras',
      'dim.agh.float1.desc': 'Millimeter precision',
      'dim.agh.float2.title': 'Argentine manufacturing',
      'dim.agh.float2.desc': 'Guaranteed quality and reliability',
      'dim.cubiscan.label': 'World leader in dimensioning',
      'dim.cubiscan.headline': 'Measurement that optimizes <span class="t-accent">every shipment and pallet</span>',
      'dim.cubiscan.title': 'Cubiscan',
      'dim.cubiscan.desc': 'Cubiscan solutions for package and pallet measurement. World-class dimensioning and weighing systems for high-performance logistics operations.',
      'dim.cubiscan.cta': 'View Cubiscan equipment',
      'dim.cubiscan.cta2': 'Watch video',
      'dim.cubiscan.f1.title': '3D measurement',
      'dim.cubiscan.f1.desc': 'Precise dimensions in seconds',
      'dim.cubiscan.f2.title': 'Easy integration',
      'dim.cubiscan.f2.desc': 'Compatible with WMS and ERP',
      'dim.cubiscan.f3.title': 'Flexibility',
      'dim.cubiscan.f3.desc': 'Equipment for all types of cargo',
      'dim.cubiscan.f4.title': 'Reliability',
      'dim.cubiscan.f4.desc': 'Robust equipment for 24/7 operation',
      'dim.cubiscan.float1.title': 'Precise 3D measurement',
      'dim.cubiscan.float1.desc': 'Precise volume and weight',
      'dim.cubiscan.float2.title': 'Full integration',
      'dim.cubiscan.float2.desc': 'Connected to your WMS and ERP',
      'dim.conlida.label': 'High-performance measurement solutions',
      'dim.conlida.headline': 'Efficiency and precision <span class="t-accent">for high volume</span>',
      'dim.conlida.title': 'Conlida',
      'dim.conlida.desc': 'Dynamic and static measurement solutions for high-volume operations with excellent cost-benefit ratio and local support in Argentina.',
      'dim.conlida.cta': 'View Conlida equipment',
      'dim.conlida.cta2': 'Watch video',
      'dim.conlida.f1.title': 'Dynamic measurement',
      'dim.conlida.f1.desc': 'Capture in motion without stopping the line',
      'dim.conlida.f2.title': 'Cost-benefit',
      'dim.conlida.f2.desc': 'Affordable investment with fast ROI',
      'dim.conlida.f3.title': 'Local support',
      'dim.conlida.f3.desc': 'Technical assistance in Argentina',
      'dim.conlida.f4.title': 'Full integration',
      'dim.conlida.f4.desc': 'Compatible with WMS, TMS and ERP',
      'dim.conlida.float1.title': 'Dynamic measurement',
      'dim.conlida.float1.desc': 'Without stopping operations',
      'dim.conlida.float2.title': 'Support in Argentina',
      'dim.conlida.float2.desc': 'Local technical assistance',
      'dim.cubetape.label': 'High-precision portable measurement',
      'dim.cubetape.headline': 'Measure anywhere with <span class="t-accent">agility and precision</span>',
      'dim.cubetape.title': 'CubeTape',
      'dim.cubetape.desc': 'CubeTape is the mobile solution to quickly capture package and parcel dimensions, transmitting data to your system to optimize shipping and freight costs.',
      'dim.cubetape.cta': 'View CubeTape equipment',
      'dim.cubetape.cta2': 'Watch video',
      'dim.cubetape.f1.title': 'Portable measurement',
      'dim.cubetape.f1.desc': 'Measure wherever you are, no fixed equipment needed',
      'dim.cubetape.f2.title': 'Fast capture',
      'dim.cubetape.f2.desc': 'Length, width and height in seconds',
      'dim.cubetape.f3.title': 'Mobile connectivity',
      'dim.cubetape.f3.desc': 'Data via Bluetooth to your app or system',
      'dim.cubetape.f4.title': 'Ideal for shipping',
      'dim.cubetape.f4.desc': 'Perfect for logistics operators and couriers',
      'dim.cubetape.float1.title': 'CubeTape technology',
      'dim.cubetape.float1.desc': 'Professional portable precision',
      'dim.cubetape.float2.title': 'Easy to use',
      'dim.cubetape.float2.desc': 'Ergonomic and lightweight design',
      'dim.cta.text': "Not sure which equipment you need? Our specialists will help you find the best option for your operation.",
      'dim.cta.btn': 'Talk to a specialist',

      'dim.equipos.label': 'Equipment catalog',
      'dim.equipos.back': 'Back to Volumetry',
      'dim.equipos.download': 'Download catalog',
      'dim.equipos.details': 'View details',
      'dim.equipos.prevImage': 'Previous image',
      'dim.equipos.nextImage': 'Next image',
      'dim.equipos.datasheet': 'Datasheet',
      'dim.equipos.video': 'Watch video',
      'dim.equipos.requestVideo': 'Request video',
      'dim.equipos.specs': 'Specifications',
      'dim.equipos.highlights': 'Highlights',
      'dim.equipos.close': 'Close',
      'dim.equipos.detailNote': 'Need a quote, demo, or WMS integration? Contact us and we’ll advise you.',
      'dim.equipos.brochure': 'Brochure',
      'dim.equipos.viewBrochure': 'View brochure',
      'dim.equipos.consult': 'Inquire',
      'dim.equipos.cs25.f1': 'Ideal for e-commerce and desktop stations',
      'dim.equipos.cs25.f2': 'Fast weight and volume capture in one step',
      'dim.equipos.cs25.f3': 'Easy integration via USB or serial port',
      'dim.equipos.cs100.f1': 'Market reference in distribution centers',
      'dim.equipos.cs100.f2': 'High precision and repeatability on every scan',
      'dim.equipos.cs100.f3': 'USB and Ethernet connectivity for your WMS',
      'dim.equipos.cs110.f1': 'Modern successor to the Cubiscan 100 for distribution centers',
      'dim.equipos.cs110.f2': 'Precise measurement with integrated display',
      'dim.equipos.cs110.f3': 'Compatible with mobile cart and WMS integration',
      'dim.equipos.cs150.f1': 'Ideal for medium and large parcels in warehouses',
      'dim.equipos.cs150.f2': 'Industrial platform with overhead sensor',
      'dim.equipos.cs150.f3': 'Ready for WMS and management system integration',
      'dim.equipos.cs200.f1': 'Built for larger parcels and freight pieces',
      'dim.equipos.cs200.f2': 'Reliable static operation in warehouses',
      'dim.equipos.cs200.f3': 'Data output ready for management systems',
      'dim.equipos.cs325.f1': 'Dimensions irregular parts and shapes',
      'dim.equipos.cs325.f2': 'Infrared LED technology for higher precision',
      'dim.equipos.cs325.f3': 'Measurement in 3–7 seconds',
      'dim.equipos.odc.f1': '3D camera for objects and parcels',
      'dim.equipos.odc.f2': 'High precision without contacting the product',
      'dim.equipos.odc.f3': 'VOXEL CAM interface',
      'dim.equipos.pdc.f1': 'Pallet dimensioning for warehouses',
      'dim.equipos.pdc.f2': 'Ideal for cross-dock and industrial ops',
      'dim.equipos.pdc.f3': 'VOXEL CAM interface',
      'dim.equipos.pdl.f1': 'LiDAR technology for pallets',
      'dim.equipos.pdl.f2': 'Measurement in under 12 seconds',
      'dim.equipos.pdl.f3': 'VOXEL CAM interface',
      'dim.equipos.cld.f1': 'Built for high-throughput lines',
      'dim.equipos.cld.f2': 'In-motion measurement on conveyors',
      'dim.equipos.cld.f3': 'Excellent cost-performance ratio',
      'dim.equipos.ct.f1': 'Portable measuring anywhere in the operation',
      'dim.equipos.ct.f2': 'Capture length, width and height in seconds',
      'dim.equipos.ct.f3': 'Send data over Bluetooth to your system',
      'dim.equipos.help': 'Not sure which one to choose? Our specialists can help you find the ideal dimensioner for your operation.',
      'dim.equipos.help.cta': 'Talk to a specialist',
      'dim.equipos.filter.all': 'All',
      'dim.equipos.filter.static': 'Static',
      'dim.equipos.filter.dynamic': 'Dynamic',
      'dim.equipos.filter.desktop': 'Desktop',
      'dim.equipos.filter.industrial': 'Industrial',
      'dim.equipos.filter.pallets': 'Pallets',
      'dim.equipos.filter.belt': 'Conveyor',
      'dim.equipos.sort.label': 'Sort by:',
      'dim.equipos.sort.featured': 'Best sellers',
      'dim.equipos.sort.name': 'Name',
      'dim.equipos.trust1.title': 'Precision technology',
      'dim.equipos.trust1.desc': 'Reliable and repeatable measurements.',
      'dim.equipos.trust2.title': 'Full integration',
      'dim.equipos.trust2.desc': 'Compatible with WMS, TMS and automation systems.',
      'dim.equipos.trust3.title': 'Specialized support',
      'dim.equipos.trust3.desc': 'Technical guidance before and after purchase.',
      'dim.equipos.trust4.title': 'Logintec backing',
      'dim.equipos.trust4.desc': 'Over 15 years delivering logistics solutions.',
      'dim.equipos.agh.pageTitle': 'AGH Equipment – Logintec',
      'dim.equipos.agh.meta': 'AGH RGB 3D camera dimensioning equipment. Logintec catalog.',
      'dim.equipos.agh.title': 'AGH equipment',
      'dim.equipos.agh.desc': 'High-precision RGB 3D camera dimensioners for demanding industrial environments.',
      'dim.equipos.agh.count': '3 available units',
      'dim.equipos.agh.odc55': 'RGB 3D camera dimensioner for fast volume, weight and dimension measurement in industrial operations.',
      'dim.equipos.agh.odc': '<span class="eq-card__text-title">Object Dimensioner Camera</span><span class="eq-card__text-desc">precise measurement of objects and parcels.</span>',
      'dim.equipos.agh.pdc': '<span class="eq-card__text-title">Pallet Dimensioner Camera</span><span class="eq-card__text-desc">true volumetry of pallets for warehouses and industrial operations.</span>',
      'dim.equipos.agh.pdl': '<span class="eq-card__text-title">Pallet Dimensioner LiDAR</span><span class="eq-card__text-desc">pallet dimensioning with LiDAR technology for complex geometries.</span>',
      'dim.equipos.agh.p1': 'Ideal for precise measurement of medium-sized packages.',
      'dim.equipos.agh.p2': 'Designed for high-volume operations and demanding environments.',
      'dim.equipos.agh.p3': 'In-motion measurement without stopping the production line.',
      'dim.equipos.agh.p4': 'Compact solution for small spaces and e-commerce.',
      'dim.equipos.agh.p5': 'Pallet dimensioning and weighing for warehouses and cross-dock.',
      'dim.equipos.agh.p6': 'Conveyor integration for continuous flow.',
      'dim.equipos.cubiscan.pageTitle': 'Cubiscan Equipment – Logintec',
      'dim.equipos.cubiscan.meta': 'Cubiscan dimensioning and weighing equipment. Logintec catalog.',
      'dim.equipos.cubiscan.title': 'Cubiscan equipment',
      'dim.equipos.cubiscan.desc': 'Dimensioning and weighing systems for high-performance parcels, packages and pallets.',
      'dim.equipos.cubiscan.count': '6 available units',
      'dim.equipos.conlida.pageTitle': 'Conlida Equipment – Logintec',
      'dim.equipos.conlida.meta': 'Conlida dimensioning equipment. Logintec catalog.',
      'dim.equipos.conlida.title': 'Conlida equipment',
      'dim.equipos.conlida.desc': 'Dynamic and static measurement solutions for high-volume operations.',
      'dim.equipos.conlida.count': '1 available unit',
      'dim.equipos.conlida.cld8000': 'High-throughput dimensioning system with excellent cost-benefit ratio.',
      'dim.equipos.cubetape.pageTitle': 'CubeTape Equipment – Logintec',
      'dim.equipos.cubetape.meta': 'CubeTape portable measurement equipment. Logintec catalog.',
      'dim.equipos.cubetape.title': 'CubeTape equipment',
      'dim.equipos.cubetape.desc': 'Portable dimension measurement for logistics operators and couriers.',
      'dim.equipos.cubetape.count': '1 available unit',
      'dim.equipos.cubetape.model': 'Smart tape to capture length, width and height and send data to your system.',

      'aria.openMenu': 'Open menu',
      'aria.langSwitch': 'Select language',
    },
  };

  const heroPhrases = {
    es: [
      { parts: [
        { type: 'bold', text: 'Importador y ' },
        { type: 'accent', text: 'Exportador Oficial' },
      ]},
      { parts: [
        { type: 'bold', text: 'Logística de ' },
        { type: 'accent', text: 'Repuestos' },
      ]},
      { parts: [
        { type: 'bold', text: 'Equipos de ' },
        { type: 'accent', text: 'dimensionamiento' },
      ]},
      { parts: [
        { type: 'bold', text: 'Almacenamiento ' },
        { type: 'light', text: 'y ' },
        { type: 'accent', text: 'Transporte' },
      ]},
    ],
    en: [
      { parts: [
        { type: 'bold', text: 'Importer & Exporter ' },
        { type: 'accent', text: 'of Record' },
      ]},
      { parts: [
        { type: 'bold', text: 'Service Parts ' },
        { type: 'accent', text: 'Logistics' },
      ]},
      { parts: [
        { type: 'bold', text: 'Dimensioning ' },
        { type: 'accent', text: 'equipment' },
      ]},
      { parts: [
        { type: 'bold', text: 'Warehousing ' },
        { type: 'light', text: '& ' },
        { type: 'accent', text: 'Transport' },
      ]},
    ],
  };

  let currentLang = 'es';

  function t(key) {
    return translations[currentLang][key] ?? translations.es[key] ?? key;
  }

  function getLang() {
    return currentLang;
  }

  function renderHeroPhrase(parts) {
    const classMap = { bold: 't-bold', light: 't-light', accent: 't-accent' };
    return parts
      .map((part) => `<span class="${classMap[part.type]}">${part.text}</span>`)
      .join('');
  }

  function getHeroPhrases() {
    return heroPhrases[currentLang];
  }

  function applyLanguage(lang) {
    if (!translations[lang]) return;
    currentLang = lang;
    localStorage.setItem(STORAGE_KEY, lang);
    document.documentElement.lang = lang;

    document.querySelectorAll('[data-i18n]').forEach((el) => {
      const key = el.getAttribute('data-i18n');
      if (translations[lang][key] !== undefined) {
        el.textContent = translations[lang][key];
      }
    });

    document.querySelectorAll('[data-i18n-html]').forEach((el) => {
      const key = el.getAttribute('data-i18n-html');
      if (translations[lang][key] !== undefined) {
        el.innerHTML = translations[lang][key];
      }
    });

    document.querySelectorAll('[data-i18n-placeholder]').forEach((el) => {
      const key = el.getAttribute('data-i18n-placeholder');
      if (translations[lang][key] !== undefined) {
        el.placeholder = translations[lang][key];
      }
    });

    document.querySelectorAll('[data-i18n-aria]').forEach((el) => {
      const key = el.getAttribute('data-i18n-aria');
      if (translations[lang][key] !== undefined) {
        el.setAttribute('aria-label', translations[lang][key]);
      }
    });

    const titleKey = document.querySelector('title')?.getAttribute('data-i18n');
    if (titleKey && translations[lang][titleKey]) {
      document.title = translations[lang][titleKey];
    }

    const metaDesc = document.querySelector('meta[name="description"]');
    const descKey = metaDesc?.getAttribute('data-i18n');
    if (metaDesc && descKey && translations[lang][descKey]) {
      metaDesc.setAttribute('content', translations[lang][descKey]);
    }

    document.querySelectorAll('.lang-switch__btn').forEach((btn) => {
      btn.classList.toggle('lang-switch__btn--active', btn.dataset.lang === lang);
    });

    const langSwitch = document.getElementById('langSwitch');
    if (langSwitch) {
      langSwitch.setAttribute('aria-label', translations[lang]['aria.langSwitch']);
    }

    document.dispatchEvent(new CustomEvent('languagechange', { detail: { lang } }));
  }

  function init() {
    const saved = localStorage.getItem(STORAGE_KEY);
    const browserLang = navigator.language?.startsWith('en') ? 'en' : 'es';
    applyLanguage(saved || browserLang);

    document.querySelectorAll('.lang-switch__btn').forEach((btn) => {
      btn.addEventListener('click', () => {
        applyLanguage(btn.dataset.lang);
      });
    });
  }

  return { init, applyLanguage, t, getLang, getHeroPhrases, renderHeroPhrase };
})();
